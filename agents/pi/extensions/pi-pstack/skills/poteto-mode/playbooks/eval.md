...
6. **Verify the chain from transcripts, not self-report.** Read the candidate session file returned by the subagent result when available. For parent sessions, use `$PI_SESSION_FILE` or `pstack_sessions`; never glob Pi's global session directory. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
