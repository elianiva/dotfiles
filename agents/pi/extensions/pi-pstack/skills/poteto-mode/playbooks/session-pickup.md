...
1. Locate the prior trail. Use `$PI_SESSION_FILE` for the active transcript or `pstack_sessions` to list another session for this working directory. A pushed branch is also valid evidence. Read the metadata overview and last messages first, then scan back for decision points. Parse a long transcript in a subagent and keep only its reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
