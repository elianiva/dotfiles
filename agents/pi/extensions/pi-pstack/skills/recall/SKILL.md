...
The active Pi session file is available to child commands as `$PI_SESSION_FILE`. To inspect earlier sessions for this same working directory, call `pstack_sessions` with `action: "list"`; it uses Pi's session API rather than guessing a path or scanning another project.
