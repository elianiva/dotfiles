...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, the current working directory, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCPs from the Pi environment. Use the available-tools map when present. Otherwise inspect the `mcps/` directory Pi exposes for enabled MCP servers.
