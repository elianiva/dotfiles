...
function todo_widget:update_counter(todos)
  ...
  for _, p in ipairs(todos) do
    if not p.status then todo_count = todo_count + 1 end
  end
...
local add_button = wibox.widget{
  ...
  shape = function(cr, width, height) gears.shape.circle(cr, width, height, 12) end,
  ...
  awful.prompt.run{
    ...
    exe_callback = function(input_text)
      ...
      spawn.easy_async(GET_TODO_ITEMS, function(stdout)
        ...
        spawn.easy_async_with_shell("echo '" .. json.encode(res) .. "' > " ..
                                      STORAGE, function()
...
add_button:connect_signal("mouse::enter",
  function(c) c:set_bg(theme.bg_focus) end)
...
local function worker(args)
  ...
  todo_widget.widget:buttons(awful.util.table.join(
