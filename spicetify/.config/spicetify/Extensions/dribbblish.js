...
new Spicetify.Menu.Item(window.__spotify.username, false, () =>
  window.open(window.__spotify.userUri)
).register()

waitForElement(
  [".LeftSidebar", ".LeftSidebar__section--rootlist .SidebarList__list"],
  queries => {
    ...
    new MutationObserver(loadPlaylistImage).observe(queries[1], {
      childList: true
    })
    ...
    */
    ...
    queries[0]
      .querySelectorAll(
        ".LeftSidebar__section:not(.LeftSidebar__section--rootlist) [href]"
      )
      .forEach(item => {
        let icon = (app => {
          switch (app) {
            ...
            case "collection:artists":
              return "artist"
            case "collection:podcasts":
              return "podcasts"
            case "playlist:local-files":
              return "localfile"
            case "stations":
              return "stations"
          }
